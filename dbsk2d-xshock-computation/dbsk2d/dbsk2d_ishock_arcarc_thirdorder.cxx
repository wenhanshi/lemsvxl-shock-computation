// This is brcv/shp/dbsk2d/dbsk2d_ishock_arcarc_thirdorder.cxx
...
dbsk2d_ishock_arcarc_thirdorder::
...
dbsk2d_ishock_edge (newid, stime, pse, lbe, rbe)
{
  ...
  _origin = ((dbsk2d_ishock_barc*)lbe)->center();
...
bool dbsk2d_ishock_arcarc_thirdorder::isLSTauValid ()
{
  if (_nu==1)
...
bool dbsk2d_ishock_arcarc_thirdorder::isRSTauValid ()
{
  if (_nu==1)
    return AisGEq(_LsTau,0) && AisLEq(_LsTau,vnl_math::pi);
  else
...
void dbsk2d_ishock_arcarc_thirdorder::compute_tau_ranges()
{
  if (_nu==1){
    _minLTau = _LsTau;
...
void dbsk2d_ishock_arcarc_thirdorder::correct_intrinsic_parameters_at_init()
{
}

void dbsk2d_ishock_arcarc_thirdorder::set_end_taus_at_init()
{
  if (_nu==1){
    _LeTau = _maxLTau;
    _ReTau = _maxRTau;
...
double dbsk2d_ishock_arcarc_thirdorder::EtaToTau(double eta, DIRECTION dir, bool constrained) 
{ 
  if (dir == LEFT)
...
double dbsk2d_ishock_arcarc_thirdorder::LEtaToLTau(double eta, bool constrained)
{
  //locate extrinsic vector first
  VECTOR_TYPE lvec = this->lBArc()->eta_to_vec(eta); //locate extrinsic vector first
...
double dbsk2d_ishock_arcarc_thirdorder::REtaToRTau(double eta, bool constrained)
{
  //locate extrinsic vector first
...
double dbsk2d_ishock_arcarc_thirdorder::LTauToLEta(double tau, bool start)
{
...
double dbsk2d_ishock_arcarc_thirdorder::RTauToREta(double tau, bool start)
{
  dbsk2d_assert(AisBetween(tau, _minRTau, _maxRTau));

  double reta = this->rBArc()->vec_to_eta(_u+tau);
...
double dbsk2d_ishock_arcarc_thirdorder::rp (double tau)
{
   return 0;
}

double dbsk2d_ishock_arcarc_thirdorder::rpp(double tau)
{
   return 0;
}

double dbsk2d_ishock_arcarc_thirdorder::g  (double tau)
{
   return 0;
}

double dbsk2d_ishock_arcarc_thirdorder::tangent (double ltau)
{
  return angle0To2Pi(_u+ltau+vnl_math::pi_over_2);
}

double dbsk2d_ishock_arcarc_thirdorder::k  (double tau)
{
   return 0;
}

double dbsk2d_ishock_arcarc_thirdorder::v  (double tau)
{
   return 100000;
}

double dbsk2d_ishock_arcarc_thirdorder::a  (double tau)
{
   return 0;
}

double dbsk2d_ishock_arcarc_thirdorder::phi (double tau)
{
  return 0;
}
...
vgl_point_2d<double> dbsk2d_ishock_arcarc_thirdorder::getPtFromLTau (double ltau)
{
  double R = (_Rl+_Rr)/2;
...
vgl_point_2d<double> dbsk2d_ishock_arcarc_thirdorder::getLFootPt (double ltau)
{
  return _translatePoint ( lBArc()->center(), angle0To2Pi(_u+ltau), lBArc()->R());
}

//always use the svcl_tandard tau
vgl_point_2d<double> dbsk2d_ishock_arcarc_thirdorder::getRFootPt (double rtau)
{
  return _translatePoint ( rBArc()->center(), angle0To2Pi(_u+rtau), rBArc()->R());
}

void dbsk2d_ishock_arcarc_thirdorder::compute_extrinsic_locus()
{
...
void dbsk2d_ishock_arcarc_thirdorder::getInfo (vcl_ostream& ostrm)
{
  char s[1024];
  char endx[32], endy[32], simtime[32], endtime[32];

  ostrm << "\n==============================\n";
  ostrm << "A-A-TO: [" << _id << "] " << "{ "  << (_bActive ? "A" : "nA" ) << ", ";
